...
@section('content')
    <div class="p-6 lg:p-8">
        <div class="flex items-center gap-4 mb-8">
            ...
            <h1 class="text-2xl font-bold text-gray-800">Edit Moment</h1>
        </div>

        <form action="{{ route('admin.moments.update', $moment) }}" method="POST" class="bg-white rounded-2xl shadow-sm p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div class="lg:col-span-2">
                    ...
                    <input type="url" id="image" name="image" value="{{ $moment->image }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4A373]/50 focus:border-[#D4A373]" placeholder="https://images.unsplash.com/photo-...">
                    <p class="text-sm text-gray-500 mt-1">Enter direct URL to image</p>
                    <div class="mt-2 p-4 bg-gray-50 rounded-xl">
                        <img id="image-preview" src="{{ $moment->image }}" alt="Preview" class="w-full max-w-sm max-h-48 rounded-xl object-cover">
                    </div>
                </div>

                <div>
                    <label for="caption" class="block text-sm font-semibold text-gray-700 mb-2">Caption *</label>
                    <input type="text" id="caption" name="caption" value="{{ $moment->caption }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4A373]/50 focus:border-[#D4A373]" placeholder="Enter caption" maxlength="255">
                ...
                <div>
                    <label class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition-colors group {{ $moment->is_featured ? 'bg-blue-50 ring-2 ring-blue-200' : '' }}">
                        ...
                        <span class="text-gray-700 font-medium group-hover:text-[#D4A373]">Featured Moment</span>
                    </label>
                    <p class="text-sm text-gray-500 mt-1">Featured moments get priority display</p>
                </div>
            </div>
